...
namespace GrafikoMat.Views
{
    public sealed partial class DashboardView : UserControl
    {
        ...
        {
            InitializeComponent();
            LeftTableHost.SizeChanged += (s, e) => BuildLeftTable();
        }
        ...
        {
            if (_vm is null || LeftTableHost.ActualWidth == 0) return;
            ...
            for (int d = 1; d <= daysInMonth; d++)
            {
                ...
                var cell = new Border { BorderBrush = borderBrush, BorderThickness = isLastColumn ? new Thickness(0, 0, 0, 1) : new Thickness(0, 0, 1, 1), Background = isDayOff ? dayOffFill : null };
                Grid.SetRow(cell, 0);
            ...
            for (int r = 0; r < _vm.DoctorRows.Count; r++)
            {
                ...
                for (int d = 1; d <= daysInMonth; d++)
                {
                    ...
                    if (!entry.has || (entry.mode == Models.DayMode.Full24 && string.IsNullOrEmpty(entry.full)) || (entry.mode == Models.DayMode.Split12 && string.IsNullOrEmpty(entry.day) && string.IsNullOrEmpty(entry.night))) { content = new TextBlock { Text = "", HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center }; }
                    else if (entry.mode == Models.DayMode.Full24) { content = new TextBlock { Text = entry.full, HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center, FontSize = 14 }; }
                    else
                    {
                        var g = new Grid();
            ...
            // Na koniec, dodajemy gotową siatkę do hosta w XAML
