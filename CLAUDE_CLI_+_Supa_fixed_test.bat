...
if %errorlevel% equ 0 (
    ...
    echo [X] Git NIE jest zainstalowany
    echo.
...
if %errorlevel% equ 0 (
    ...
    echo [X] Git Bash NIE jest w PATH
    echo.
...
if %errorlevel% equ 0 (
    ...
    echo [X] Claude Code NIE jest zainstalowany
    goto :install_claude
)
...
if exist "node_modules\.bin\supabase.cmd" (
    ...
    echo [X] Supabase CLI NIE jest zainstalowany
...
if %errorlevel% equ 0 (
    ...
    echo [X] Windows Terminal NIE jest zainstalowany
    echo.
...
echo        @ = %%%%40, znak rownosci = %%%%3D, procent = %%%%25
echo.
pause
...
REM Zapisujemy config bez uzycia nawiasow
...
if "%choice%"=="6" (
    ...
    echo DROP SCHEMA public CASCADE; CREATE SCHEMA public; | psql "%SUPABASE_DB_URL%"
...
if "%choice%"=="8" (
    ...
    echo.
    echo Node.js:
    where node >nul 2>&1 && node --version || echo [X] Nie zainstalowane
    echo.
    echo Git:
    where git >nul 2>&1 && git --version || echo [X] Nie zainstalowane
    echo.
    echo Git Bash:
    where bash >nul 2>&1 && bash --version || echo [X] Nie dostepne w PATH
    echo.
    echo Claude Code:
    where claude >nul 2>&1 && claude --version || echo [X] Nie zainstalowane
    echo.
    echo Supabase CLI (lokalny):
    if exist "node_modules\.bin\supabase.cmd" (
        npx supabase --version
    ) else (
        echo [X] Nie zainstalowane
    )
    echo.
    echo Windows Terminal:
    where wt.exe >nul 2>&1 && echo [OK] Zainstalowane || echo [X] Nie zainstalowane
    echo.
